...
def objective_time_prune(trial, X, y, kf,
    ...
    n_out = trial.suggest_int("n_out", 2, 20)
    mean_type = trial.suggest_categorical("mean_type", ["constant", "linear"])
    kernel_type = trial.suggest_categorical("kernel_type", ["rbf", "matern0.5", "matern1.5"])  # issue with spectral mixture kernel for VI
    lr = trial.suggest_float("lr", 1e-4, 1e-1, log=True)
    batch_size = trial.suggest_categorical("batch_size", [512, 1024, 2048])
    num_inducing = trial.suggest_int("num_inducing", 50, 2000, log=True)
    initialize_fe = trial.suggest_categorical("initialize_fe", [True, False])
    minit = trial.suggest_categorical("minit", ["random", "kmeans"])

    print(
        f'Current Configuration: n_out:{n_out} - n_max: {n_max} - n_layer: {n_layers} - mean_type: {mean_type} - \
           kernel_type: {kernel_type} - lr: {lr} - num_inducing: {num_inducing}')
    ...
    for epoch in range(epochs_inner):
        ...
        for train_inner, val_inner in kf.split(X):
            ...
            temp_model_name = './models/temp/VDKL_temp_split_' + str(cur_split_nr) + '.pt'
            ...
            model = VDKLRegression(X_train_inner, y_train_inner, feature_extractor,
                                   num_inducing=num_inducing, mean_type=mean_type,
                                   kernel_type=kernel_type,  minit=minit)
            ...
            mll = gpytorch.mlls.VariationalELBO(likelihood, model, num_data=len(train_inner_loader.dataset))
            ...
            val_means = torch.tensor([0.])

            with torch.no_grad():
                for x_batch, y_batch in test_inner_loader:
                    preds = model(x_batch)
                    val_means = torch.cat([val_means, preds.mean.cpu()])

            val_means = val_means[1:]
            valid_loss = metric(val_means, y_val_inner.cpu())
            ...
            del X_train_inner, y_train_inner, X_val_inner, y_val_inner, loss, optimizer, model, output, mll
            gc.collect()
            torch.cuda.empty_cache()
...
def objective_train_test(trial, X, y, epochs_inner, patience, time_tolerance=1800, metric=mse):
    ...
    batch_size = trial.suggest_categorical("batch_size", [1024, 2048, 8192])
    num_inducing = trial.suggest_int("num_inducing", 50, 2000, log=True)
    initialize_fe = trial.suggest_categorical("initialize_fe", [True, False])
    minit = trial.suggest_categorical("minit", ["random", "kmeans"])
    ...
    for epoch in range(epochs_inner):
        ...
        if epoch % 10 == 0:
            print(f"{epoch}/{epochs_inner} - Loss: {loss} - Score: {valid_loss}")
    ...
    logging.info(torch.cuda.memory_allocated() / 1024 ** 2)
    logging.info(torch.cuda.memory_reserved() / 1024 ** 2)
    clear_cuda_memory(X_train_inner, y_train_inner, X_val_inner, y_val_inner,
...
def HPO_VDKL(n_splits_outer=5,
    ...
    # File Names
    log_filename = "./log/HPO_VDKL_log_" + dataset_name + ".log"
    savedir = './data/experiments/VDKL'
    if not os.path.exists(savedir):
        os.mkdir(savedir)
    study_infos_file = savedir + '/study_infos_VDKL_' + dataset_name + '.csv'
    ...
    for train_outer, test_outer in kf_outer.split(np.array(X)):
        ...
        try:
            ...
            study_infos = pd.DataFrame({'dataset_id': [], 'dataset_name': [], 'outer_cv_split': [],
                                        ...
                                        'params_lr': [], 'params_batch_size': [], 'params_num_inducing': [],
                                        'params_initialize_fe': [], 'params_minit': [],
                                        'user_attrs_MAX_EPOCHS': [], 'state': []})
        ...
        try:
            ...
            nested_resampling_infos = pd.DataFrame({'dataset_id': [], 'dataset_name': [], 'outer_run': [],
                                                    'n_max': [], 'n_layers': [],
                                                    'n_out': [], 'mean_type': [],
                                                    'kernel_type': [], 'num_mixtures': [],
                                                    'lr': [], 'batch_size': [], 'num_inducing': [],
                                                    'initialize_fe': [], 'minit': [],
                                                    'MAX_EPOCHS': [], 'mse': [], 'mae': [], 'max_absolute_error': [],
        ...
        if pruner_type == "HB":
            pruner = optuna.pruners.HyperbandPruner(min_resource=5, max_resource=epochs_inner, reduction_factor=3)
        ...
        study_name = "Nested Resampling VDKL " + str(split_nr)
        study = optuna.create_study(direction="minimize",
                                    study_name=study_name,
                                    pruner=pruner)
        # Suggest default parameters
        study.enqueue_trial({"n_max": 1024, 'n_layers': 4, 'n_out': 8, 'mean_type': 'constant', 'kernel_type': 'rbf',
                             'lr': 0.01, 'num_inducing': 1000, 'initialize_fe': False, 'params_minit': "random"})
        try:
        ...
        minit_best = best_trial.params['minit']
        ...
        # save_path = './models/' + 'VDKL_' + dataset_name + str(split_nr) + '.pt'
